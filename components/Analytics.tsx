...
const Analytics: React.FC<AnalyticsProps> = ({ data, onStudentClick }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      {/* 1. Header & Selector */}
      <div className="flex flex-col md:flex-row justify-between items-end gap-4 bg-slate-900/50 backdrop-blur-md p-6 rounded-2xl border border-slate-800">
        <div>
      ...
      <div className="bg-slate-900/50 backdrop-blur-md border border-red-900/30 p-6 rounded-2xl shadow-lg mt-6">
        <h3 className="text-lg font-bold text-white mb-6 flex items-center">
            ...
            <table className="w-full text-left border-collapse">
                <thead>
                    <tr className="bg-slate-950/80 text-slate-400 text-xs uppercase tracking-wider border-b border-slate-800">
                        ...
                        <th className="p-4 text-center">Kekerapan</th>
                ...
                <tbody className="divide-y divide-slate-800">
                    {monthlyStats?.topStudents.map((s, idx) => (
                        <tr key={idx} className="hover:bg-slate-800/40 transition-colors group cursor-pointer" onClick={() => onStudentClick(s.name)}>
                            ...
                            <td className="p-4 font-medium text-slate-200 group-hover:text-cyan-400 transition-colors underline decoration-dotted decoration-slate-600 underline-offset-4">{s.name}</td>
                            <td className="p-4 text-xs text-cyan-400 font-mono">{s.id}</td>
                            <td className="p-4 text-xs text-slate-400">{s.class}</td>
