...
const StudentList: React.FC<StudentListProps> = ({ data, onStudentClick }) => {
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="bg-slate-900/50 backdrop-blur-md border border-cyan-900/30 p-6 rounded-2xl shadow-lg">
        {/* Header Controls */}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-950/80 text-slate-400 text-xs uppercase tracking-wider border-b border-slate-800">
                <th className="p-4 font-medium">Tarikh / Masa</th>
                <th className="p-4 font-medium">ID Pelajar</th>
                <th className="p-4 font-medium">Nama Pelajar</th>
                <th className="p-4 font-medium">Kelas</th>
                <th className="p-4 font-medium text-right">Masa Lewat</th>
              </tr>
            </thead>
              ...
              {filteredData.length > 0 ? (
                filteredData.map((row) => (
                  <tr key={row.id} className="hover:bg-cyan-900/10 transition-colors group">
                    ...
                    <td className="p-4 text-sm text-cyan-400 font-mono">{row.studentId}</td>
                    <td 
                        className="p-4 text-sm text-slate-200 font-medium group-hover:text-cyan-300 cursor-pointer underline decoration-dotted underline-offset-4 decoration-slate-600 hover:decoration-cyan-400"
                    ...
                    <td className="p-4 text-sm text-right font-mono text-red-400 font-bold">
                      +{row.minutesLate} min
                    </td>
                  </tr>
                ))
